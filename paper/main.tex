...
times the baseline. In the three-objective outcome space (maximize balanced
accuracy and stability; minimize feature count), the full method is Pareto
non-dominated on both datasets and dominates SF-EHOA on Wine. The evidence
therefore supports a scoped stability--reduction trade-off, not universal predictive superiority. All 80
...
\subsection{Research hypotheses and novelty boundary}
The study tests three preregisterable, falsifiable hypotheses: $H_1$, feedback
increases chance-corrected subset stability relative to EHOA; $H_2$, interaction
guidance improves the held-out performance--reduction trade-off; and $H_3$, the
full method is Pareto non-dominated in balanced accuracy (maximize), Nogueira
stability (maximize), and selected-feature count (minimize). Failure to improve
accuracy alone does not confirm these hypotheses. The novelty claim is limited
to the coupled mechanism: cross-resampling evidence controls continuous search
dynamics through a corroborated two-regime law and controls discrete inclusion
through a separate entropy-confidence gate. Neither adaptive metaheuristics nor
relevance/redundancy scoring is claimed as novel in isolation.
...
\subsection{Pareto and hypothesis audit}
A method is dominated only if another method is no worse on all three reported
objectives and strictly better on at least one. DFA-EHOA is non-dominated on
both datasets. On Breast Cancer, SF-EHOA has higher accuracy and fewer features,
but lower Nogueira stability; hence neither dominates the other. On Wine,
IG-EHOA has slightly higher accuracy and stability but uses more features,
whereas EHOA uses fewer features with lower stability. DFA-EHOA also strictly
dominates SF-EHOA on Wine. Accordingly, $H_1$ is supported descriptively for the
full method on both datasets, $H_2$ receives component-level but inconsistent
support, and $H_3$ is supported for this limited benchmark. None is promoted to
a population-wide or state-of-the-art claim because corrected predictive tests
are non-significant and only two datasets were available.
...
The exact CC BY 4.0 dataset snapshots are committed with source attribution and
checksums; no private records are required. The software is MIT licensed
